...
int main(int argc, char **argv)
{
    ...
    message_filters::Subscriber<sensor_msgs::Image> left_sub(nh, "/camera/infra1/image_rect_raw", 50);
    message_filters::Subscriber<sensor_msgs::Image> right_sub(nh, "/camera/infra2/image_rect_raw", 50);
    typedef message_filters::sync_policies::ApproximateTime<sensor_msgs::Image, sensor_msgs::Image> sync_pol;
    message_filters::Synchronizer<sync_pol> sync(sync_pol(10), left_sub,right_sub);
    sync.registerCallback(boost::bind(&ImageGrabber::GrabStereo,&igb,_1,_2));

    CamPose_Pub = nh.advertise<geometry_msgs::PoseStamped>("/mavros/vision_pose/pose",50);
    Camodom_Pub = nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("Camera_Odom", 50);
    tracking_img_pub = image_transport.advertise( "FRP_SLAM/tracking_image", 50);
    tracked_mappoints_pub = nh.advertise<sensor_msgs::PointCloud2>( "FRP_SLAM/tracked_points", 50);
    all_mappoints_pub = nh.advertise<sensor_msgs::PointCloud2>( "FRP_SLAM/all_points", 50);
...
void ImageGrabber::GrabStereo(const sensor_msgs::ImageConstPtr& msgLeft,const sensor_msgs::ImageConstPtr& msgRight)
{
    ...
    if(do_rectify)
    {
        ...
        if (sophus_Tcw == Sophus::SE3f::Identity())
        {
            // 获取旋转矩阵和平移向量
            Eigen::Matrix3f RWC = sophus_Tcw.rotationMatrix();
            Eigen::Vector3f tWC = sophus_Tcw.translation();
