...
int main(int argc, char **argv)
{
    ...
    if(argc != 3)
    {
        cerr << endl << "Usage: rosrun FRP_SLAM Mono path_to_vocabulary path_to_settings" << endl;
        ros::shutdown();
        return 1;
    }

    // Create SLAM system. It initializes all system threads and gets ready to process frames.
    FRP_SLAM::System SLAM(argv[1],argv[2],FRP_SLAM::System::MONOCULAR,false);
    ...
    thread tViewer = thread(&FRP_SLAM::ViewerAR::Run,&viewerAR);
...
void ImageGrabber::GrabImage(const sensor_msgs::ImageConstPtr& msg)
{
    ...
    vector<FRP_SLAM::MapPoint*> vMPs = mpSLAM->GetTrackedMapPoints();
